...
def get_caption_model_processor(
    model_name: str, model_name_or_path: str, device: str
) -> Dict[str, Any]:
...
def get_som_labeled_img(
    ...
    for i, box in enumerate(boxes):
        ...
        inputs = processor(images=region_pil, return_tensors="pt").to(model.device)

        out = model.generate(**inputs, max_new_tokens=20, num_beams=5)
